...
function display_loobo_menu() {
    add_menu_page('主题设置', '主题设置', 'administrator','barley', 'loobo_setting_page','dashicons-hammer',29);
...
function loobo_admin_get_field_html($field){
	...
	switch ($type) {
		...
		case 'image':
			$field_html = '<div class="input-group"><input type="text" class="form-control" value="'.esc_attr($value).'" name="'.$name.'" id="'.$key.'" type="url"><span class="input-group-btn"><input type="button" class="loobo_upload btn btn-info" value="上传"></span></div>';
